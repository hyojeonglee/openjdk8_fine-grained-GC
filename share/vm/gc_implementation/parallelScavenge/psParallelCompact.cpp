...
// hjlee - Add record_obj_info()
void ParallelCompactData::record_obj_info(HeapWord* start_addr, size_t old_space_total_live) {
	size_t dense_prefix_end = addr_to_region_idx(start_addr);

	// for (dense_prefix_end < i < _region_count)
	// (compaction 대상이 되는 모든 리전을 대상으로)
	// 
	// endfor
}
...
PSParallelCompact::summarize_space(SpaceId id, bool maximum_compaction)
{
  ...
  if (_space_info[id].new_top() != space->bottom()) {
	...
	// hjlee - Call here
	_summary_data.record_obj_info(dense_prefix_end, old_space_total_live);
                                                     ...
                                                     HeapWord* end_addr)
{
  ...
  // TODO - new_top() equals to end point of regions which are destinations,
  // so it means that if the region is available, then push it
  // to local region stack. (it can be target for fill_region's destination)
